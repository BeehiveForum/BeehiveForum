...
/* $Id: thread_list.php,v 1.291 2007-04-12 21:13:36 decoyduck Exp $ */
...
foreach ($folder_order as $folder_number) {
    ...
    if (isset($folder_info[$folder_number]) && is_array($folder_info[$folder_number])) {
        ...
        echo "            <a href=\"thread_list.php?webtag=$webtag&amp;mode=0&amp;folder=$folder_number\" title=\"", add_wordfilter_tags($folder_info[$folder_number]['DESCRIPTION']), "\">", add_wordfilter_tags($folder_info[$folder_number]['TITLE']), "</a>\n";
        echo "          </td>\n";
                ...
                if ($visible_threads) {
                    ...
                    foreach($thread_info as $thread) {
                        ...
                        if ($thread['FID'] == $folder_number) {
                            ...
                            if (isset($thread['TRACK_TYPE']) && $thread['TRACK_TYPE'] == THREAD_TYPE_SPLIT) echo "<img src=\"" . style_image('split_thread.png') . "\" alt=\"{$lang['threadsplit']}\" title=\"{$lang['threadsplit']}\" /> ";
                            if (isset($thread['TRACK_TYPE']) && $thread['TRACK_TYPE'] == THREAD_TYPE_MERGE) echo "<img src=\"" . style_image('merge_thread.png') . "\" alt=\"{$lang['threadmerge']}\" title=\"{$lang['threadmerge']}\" /> ";
...
if ($mode == ALL_DISCUSSIONS && !isset($folder)) {
    ...
    if (is_array($folder_msgs)) {
        ...
        if ($more_threads > 0 && $more_threads <= 50) {
            ...
            echo "</tr>\n";
            echo "<tr>\n";
            echo "  <td align=\"left\" class=\"smalltext\" colspan=\"2\"><img src=\"".style_image('current_thread.png')."\" alt=\"{$lang['next']} $more_threads {$lang['threads']}\" title=\"{$lang['next']} $more_threads {$lang['threads']}\" />&nbsp;<a href=\"thread_list.php?webtag=$webtag&amp;mode=0&amp;start_from=".($start_from + 50)."\">{$lang['next']} $more_threads {$lang['threads']}</td>\n";
            echo "</tr>\n";
