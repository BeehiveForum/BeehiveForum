...
/* $Id: admin_user_groups_edit.php,v 1.2 2004-05-20 16:14:08 decoyduck Exp $ */
...
if (isset($_POST['submit'])) {
    ...
    $new_group_perms = (double) $t_banned | $t_wormed;

    if (perm_has_forumtools_access()) {

        $new_group_perms = (double)$new_group_perms | $t_admintools;
        $new_group_perms = (double)$new_group_perms | ($user_perms & USER_PERM_FORUM_TOOLS);

    }else {

        $new_group_perms = (double)$new_group_perms | ($user_perms & USER_PERM_ADMIN_TOOLS);
        $new_group_perms = (double)$new_group_perms | ($user_perms & USER_PERM_FORUM_TOOLS);
    }

    if ($new_group_perms != $group_permissions) {

        perm_group_update_permissions($gid, $new_group_perms);
        admin_addlog($uid, 0, 0, 0, 0, 0, 1);
    }

    $updated_folder_perms = false;
    ...
    if (isset($_POST['t_new_perms_array']) && is_array($_POST['t_new_perms_array'])) {
        ...
        foreach ($t_new_perms_array as $fid) {
            ...
            perm_group_add_folder_perms($gid, $fid, $new_group_perms);
        }
    }
...
if ($folder_array = folder_get_all()) {
    ...
    foreach($folder_array as $folder) {
        ...
        if ($group_folder_permissions = perm_get_group_folder_perms($gid, $folder['FID'])) {
            ...
            echo "                                      <td nowrap=\"nowrap\">", form_checkbox("t_post_read[{$user_folder_permissions['GID']}]", USER_PERM_POST_READ, "Read Posts", $group_folder_permissions & USER_PERM_POST_READ), "</td>\n";
            echo "                                      <td nowrap=\"nowrap\">", form_checkbox("t_post_create[{$user_folder_permissions['GID']}]", USER_PERM_POST_CREATE, "Reply to threads", $group_folder_permissions & USER_PERM_POST_CREATE), "</td>\n";
            echo "                                    </tr>\n";
            echo "                                    <tr>\n";
            echo "                                      <td width=\"100\">&nbsp;</td>\n";
            echo "                                      <td nowrap=\"nowrap\">", form_checkbox("t_thread_create[{$user_folder_permissions['GID']}]", USER_PERM_THREAD_CREATE, "Create new threads", $group_folder_permissions & USER_PERM_THREAD_CREATE), "</td>\n";
            echo "                                      <td nowrap=\"nowrap\">", form_checkbox("t_post_edit[{$user_folder_permissions['GID']}]", USER_PERM_POST_EDIT, "Edit Posts", $group_folder_permissions & USER_PERM_POST_EDIT), "</td>\n";
            echo "                                    </tr>\n";
            echo "                                    <tr>\n";
            echo "                                      <td width=\"100\">&nbsp;</td>\n";
            echo "                                      <td nowrap=\"nowrap\">", form_checkbox("t_post_delete[{$user_folder_permissions['GID']}]", USER_PERM_POST_DELETE, "Delete Posts", $group_folder_permissions & USER_PERM_POST_DELETE), "</td>\n";
            echo "                                      <td nowrap=\"nowrap\">", form_checkbox("t_post_attach[{$user_folder_permissions['GID']}]", USER_PERM_POST_ATTACHMENTS, "Upload Attachments", $group_folder_permissions & USER_PERM_POST_ATTACHMENTS), "</td>\n";
            echo "                                    </tr>\n";
            echo "                                    <tr>\n";
            echo "                                      <td width=\"100\">&nbsp;</td>\n";
            echo "                                      <td colspan=\"3\">", form_checkbox("t_moderator[{$user_folder_permissions['GID']}]", USER_PERM_MODERATOR, "Moderate Folder", $group_folder_permissions & USER_PERM_MODERATOR), "</td>\n";
            echo "                                      <td>&nbsp;</td>\n";
            echo "                                      <td>&nbsp;</td>\n";
            echo "                                    </tr>\n";
            echo "                                    <tr>\n";
