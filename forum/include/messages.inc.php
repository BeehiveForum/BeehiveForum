...
/* $Id: messages.inc.php,v 1.388 2006-04-19 20:28:47 decoyduck Exp $ */
...
* Increments VIEWCOUNT column in THREAD.
*
* @return bool
...
function messages_update_read($tid, $pid, $uid, $spid = 1, $chk_lr = true)
{
    ...
    if ($uid > 0) {
        ...
        if ($chk_lr === true) {
            ...
            if (db_num_rows($result) > 0) {
                ...
                if (!isset($fa['LAST_READ'])) $fa['LAST_READ'] = 0;
    ...
    if (forum_get_setting('show_stats', 'Y')) {
        ...
        if ($user_show_stats == "Y" || $uid == 0) {
            ...
            if ($longest_thread = get_longest_thread()) {
                ...
                echo "          <tr>\n";
                echo "            <td width=\"35\">&nbsp;</td>\n";
                echo "            <td>{$lang['longestthreadis']} '<a href=\"./index.php?webtag=$webtag&amp;msg={$longest_thread['TID']}.1\">{$longest_thread['TITLE']}</a>' {$lang['with']} <b>", number_format($longest_thread['LENGTH'], 0, ",", ","), "</b> {$lang['postslowercase']}.</td>\n";
                echo "            <td width=\"35\">&nbsp;</td>\n";
                echo "          </tr>\n";
                echo "        </table>\n";
            }
