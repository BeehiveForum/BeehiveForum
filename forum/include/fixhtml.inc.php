...
function fix_html($html, $bad_tags = array("plaintext", "applet", "body", "html", "head", "title", "base", "meta", "!doctype", "button", "fieldset", "form", "frame", "frameset", "iframe", "input", "label", "legend", "link", "noframes", "noscript", "object", "optgroup", "option", "param", "script", "select", "style", "textarea", "xmp"))
{
    ...
    // Until Andee fixes this, we'll use the old 0.3 fix_html function.
    return fix_html_old($html);
...
// OLD VERSION FROM 0.3. Until new version can be sorted, we'll use this.

function fix_html_old($html, $bad_tags = array("plaintext", "applet", "body", "html", "head", "title", "base", "meta", "!doctype", "button", "fieldset", "form", "frame", "frameset", "iframe", "input", "label", "legend", "link", "noframes", "noscript", "object", "optgroup", "option", "param", "script", "select", "style", "textarea", "xmp"))
{
                        ...
                        if($i%2){
                                if(substr($html_parts[$i],0,1) == "/"){ // closing tag
                                        ...
                                        if(in_array($tag, $bad_tags) || in_array($tag, $single_tags)){
                                                ...
                                                if(in_array($tag, array_keys($opentags)) && $opentags[$tag] > 0 && $last_tag2 == $tag){
                                                        $opentags[$tag]--;

                                                // tag hasn't been opened
                                                } else if(in_array($tag, array_keys($opentags)) && $opentags[$tag] <= 0){
                                                        $html_parts[$i-1] .= $html_parts[$i+1];
                                                        array_splice($html_parts, $i, 2);
                                                        $i--;
                                        ...
                                        if(in_array($tag, $bad_tags)){
                                                ...
                                                if (in_array($tag, array_keys($nest)) && in_array($nest[$tag][0], array_keys($opentags)) && $opentags[$nest[$tag][0]] == 0) {
                                                        $tmp_nest = $tag;
                                                        $last_tag2 = array_pop($last_tag);
                                                        $tmp_tags = array($last_tag2);
                                                        $tmp_len = $i;
                                                ...
                                                if(isset($no_nest[$tag])) {
                                                        ...
                                                        if (in_array($tag, array_keys($opentags)) && $opentags[$tag] > $opencount) {
                                                                for($j=count($last_tag)-2;$j>=0;$j--){
                                                                        if($last_tag[$j] == $tag){
                                                                                array_splice($last_tag, $j, 1);
                                                                                break;
