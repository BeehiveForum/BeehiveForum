...
/* $Id: threads.inc.php,v 1.321 2008-08-05 17:38:49 decoyduck Exp $ */
...
function threads_get_all($uid, $start = 0) // get "all" threads (i.e. most recent threads, irrespective of read or unread status).
{
    ...
    $sql.= "THREAD.POLL_FLAG, THREAD.STICKY, THREAD_STATS.VIEWCOUNT, USER_THREAD.LAST_READ, ";
    $sql.= "USER_THREAD.INTEREST, FOLDER.PREFIX, USER_FOLDER.INTEREST AS FOLDER_INTEREST, ";
    $sql.= "UNIX_TIMESTAMP(THREAD.MODIFIED) AS MODIFIED, USER.LOGON, USER.NICKNAME, ";
    $sql.= "USER_PEER.PEER_NICKNAME, USER_PEER.RELATIONSHIP, THREAD_TRACK.TRACK_TYPE ";
...
function threads_get_unread($uid) // get unread messages for $uid
{
    ...
    $sql.= "AND (USER_THREAD.INTEREST IS NULL OR USER_THREAD.INTEREST > -1) ";
    $sql.= "AND (USER_FOLDER.INTEREST IS NULL OR USER_FOLDER.INTEREST > -1) ";
    $sql.= "AND THREAD.DELETED = 'N' ORDER BY THREAD.STICKY DESC, ";
    $sql.= "THREAD.MODIFIED DESC LIMIT 0, 50";
...
function threads_get_unread_to_me($uid) // get unread messages to $uid (ignores folder interest level)
{
    ...
    $sql = "SELECT THREAD.TID, THREAD.FID, THREAD.TITLE, THREAD.DELETED, THREAD.LENGTH, ";
    $sql.= "THREAD.POLL_FLAG, THREAD.STICKY, THREAD_STATS.VIEWCOUNT, USER_THREAD.LAST_READ, ";
    $sql.= "USER_THREAD.INTEREST, UNIX_TIMESTAMP(THREAD.MODIFIED) AS MODIFIED, FOLDER.PREFIX, ";
...
function threads_get_unread_by_interest($uid, $interest = THREAD_INTERESTED) // get unread messages for $uid by interest (default High Interest)
{
    ...
    $sql.= "AND (THREAD.MODIFIED > FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - $unread_cutoff_stamp)) ";
    $sql.= "AND USER_THREAD.TID = THREAD.TID AND USER_THREAD.UID = '$uid' ";
    $sql.= "AND USER_THREAD.INTEREST = '$interest' AND THREAD.DELETED = 'N' ";
    $sql.= "AND (USER_FOLDER.INTEREST IS NULL OR USER_FOLDER.INTEREST > -1) ";
    $sql.= "ORDER BY THREAD.STICKY DESC, THREAD.MODIFIED DESC ";
    $sql.= "LIMIT 0, 50";
...
function threads_get_unread_by_relationship($uid, $relationship = USER_FRIEND) // get unread messages started by people of a particular relationship (default friend)
{
    ...
    $sql = "SELECT THREAD.TID, THREAD.FID, THREAD.TITLE, THREAD.DELETED, THREAD.LENGTH, ";
    $sql.= "THREAD.POLL_FLAG, THREAD.STICKY, THREAD_STATS.VIEWCOUNT, USER_THREAD.LAST_READ, ";
    $sql.= "USER_THREAD.INTEREST, USER_FOLDER.INTEREST AS FOLDER_INTEREST, FOLDER.PREFIX, ";
...
function threads_get_longest_unread($uid) // get unread messages for $uid
{
    ...
    $sql = "SELECT THREAD.TID, THREAD.FID, THREAD.TITLE, THREAD.DELETED, THREAD.LENGTH, ";
    $sql.= "THREAD.POLL_FLAG, THREAD.STICKY, THREAD_STATS.VIEWCOUNT, USER_THREAD.LAST_READ, ";
    $sql.= "USER_THREAD.INTEREST, USER_FOLDER.INTEREST AS FOLDER_INTEREST, ";
...
function threads_get_most_recent($limit = 10, $folder_list_array = array(), $creation_order = false)
{
    ...
    $sql.= "THREAD.LENGTH, THREAD.POLL_FLAG, USER_THREAD.LAST_READ, THREAD_STATS.VIEWCOUNT, ";
    $sql.= "UNIX_TIMESTAMP(THREAD.MODIFIED) AS MODIFIED, USER_PEER.RELATIONSHIP, ";
    $sql.= "USER_THREAD.INTEREST, USER.LOGON, USER.NICKNAME, USER_PEER.PEER_NICKNAME, ";
    $sql.= "FOLDER.PREFIX,THREAD_TRACK.TRACK_TYPE FROM {$table_data['PREFIX']}THREAD THREAD ";
    $sql.= "LEFT JOIN {$table_data['PREFIX']}THREAD_TRACK THREAD_TRACK ";
    $sql.= "ON (THREAD_TRACK.TID = THREAD.TID) ";
    $sql.= "LEFT JOIN {$table_data['PREFIX']}THREAD_STATS THREAD_STATS ";
    $sql.= "ON (THREAD_STATS.TID = THREAD.TID) ";
...
{
    if (!$db_threads_get_unread_cutoff = db_connect()) return false;

    if (($unread_cutoff_stamp = forum_get_unread_cutoff()) === false) return false;

    $sql = "SELECT UNIX_TIMESTAMP(NOW()) - $unread_cutoff_stamp";

    if (!$result = db_query($sql, $db_threads_get_unread_cutoff)) return false;

    list($thread_unread_cutoff) = db_fetch_array($result, DB_RESULT_NUM);

    return $thread_unread_cutoff;
}
