...
/* $Id: admin_wordfilter.php,v 1.30 2004-03-19 15:00:15 decoyduck Exp $ */
    ...
    if (isset($HTTP_POST_VARS['match']) && is_array($HTTP_POST_VARS['match'])) {
        foreach ($HTTP_POST_VARS['match'] as $key => $value) {
            ...
            if (isset($HTTP_POST_VARS['replace'][$key]) && trim(strlen($HTTP_POST_VARS['replace'][$key])) > 0) {
                admin_add_word_filter($HTTP_POST_VARS['match'][$key], $HTTP_POST_VARS['replace'][$i], $preg_expr);
            }else {
                admin_add_word_filter($HTTP_POST_VARS['match'][$key], "", $preg_expr);
            }
        }
    }    
