...
/* $Id: post.inc.php,v 1.148 2007-02-10 13:05:44 decoyduck Exp $ */
...
function post_create_thread($fid, $uid, $title, $poll = 'N', $sticky = 'N', $closed = false)
{
    ...
    $title = addslashes(_htmlentities($folder_prefix. $title));
...
function post_draw_to_dropdown($default_uid, $show_all = true)
{
    ...
    if (isset($default_uid) && $default_uid != 0) {
        ...
        if (db_num_rows($result) > 0) {
            ...
            if ($top_user = db_fetch_array($result)) {
                ...
                $fmt_username = add_wordfilter_tags(format_user_name($top_user['LOGON'], $top_user['NICKNAME']));
    ...
    while ($row = db_fetch_array($result)) {
        ...
        if (isset($row['LOGON']) && isset($row['NICKNAME'])) {
            ...
            $fmt_username = add_wordfilter_tags(format_user_name($row['LOGON'], $row['NICKNAME']));
