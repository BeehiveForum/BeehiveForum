...
/* $Id: session.inc.php,v 1.390 2009-09-04 22:01:45 decoyduck Exp $ */
        ...
        if (db_num_rows($result) > 0) {
            ...
            if (((time() - $user_sess['TIME']) > $active_sess_cutoff) || ($user_sess['FID'] != $forum_fid)) {
                ...
                if ($user_sess['FID'] != $forum_fid) {
                    ...
                    bh_update_visitor_log($user_sess['UID'], $forum_fid, __LINE__);
        ...
        if (db_num_rows($result) > 0) {
            ...
            if (((time() - $user_sess['TIME']) > $active_sess_cutoff) || $user_sess['FID'] != $forum_fid) {
                ...
                if ($user_sess['FID'] != $forum_fid) {
                    ...
                    bh_update_visitor_log(0, $forum_fid, __LINE__);
...
function bh_update_visitor_log($uid, $forum_fid, $line)
{
...
function bh_session_init($uid, $update_visitor_log = true, $skip_cookie = false)
{
    ...
    if ($update_visitor_log === true) {
        ...
        bh_update_visitor_log($uid, $forum_fid, __LINE__);
