...
/* $Id: edit_profile.php,v 1.113 2010-01-17 11:31:49 decoyduck Exp $ */
...
if (is_array($profile_items_array) && sizeof($profile_items_array) > 0) {
    ...
    if ($admin_edit === true) {
        ...
        html_draw_top("title={$lang['admin']} Â» {$lang['editprofile']} Â» ". format_user_name($user['LOGON'], $user['NICKNAME']));

        echo "<h1>{$lang['admin']} &raquo; {$lang['editprofile']} &raquo; ", format_user_name($user['LOGON'], $user['NICKNAME']), "</h1>\n";

    }else {

        html_draw_top("title={$lang['mycontrols']} Â» {$lang['editprofile']}");
