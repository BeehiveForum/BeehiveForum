...
/* $Id: forum.inc.php,v 1.330 2008-08-16 18:55:53 decoyduck Exp $ */
...
function forum_get_setting($setting_name, $value = false, $default = false)
{
    ...
    if (is_array($forum_settings) && isset($forum_settings[$setting_name])) {
        ...
        if ($value !== false) {
            ...
            return strtoupper($forum_settings[$setting_name]) == strtoupper($value);
    ...
    if (is_array($forum_global_settings) && isset($forum_global_settings[$setting_name])) {
        ...
        if ($value !== false) {
            ...
            return strtoupper($forum_global_settings[$setting_name]) == strtoupper($value);
        ...
        if (db_num_rows($result_forums) > 0) {
            ...
            while (($forum_data = db_fetch_array($result_forums))) {
                ...
                if (is_numeric($unread_cutoff_stamp) && $unread_cutoff_stamp !== false) {
                    ...
                    $sql = "SELECT SUM(THREAD.LENGTH) - SUM(COALESCE(USER_THREAD.LAST_READ, THREAD.UNREAD_PID)) AS UNREAD_MESSAGES ";
                    ...
                    $sql.= "AND (THREAD.MODIFIED > FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - $unread_cutoff_stamp) OR $unread_cutoff_stamp = 0) ";
