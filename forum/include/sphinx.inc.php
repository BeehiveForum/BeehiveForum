...
/* $Id: thread.inc.php 4828 2011-09-29 20:27:59Z decoyduck $ */
...
function sphinx_search_update_index($tid, $pid = false)
{
    ...
    if (!$db_sphinx_search_update_index = db_connect()) return false;

    if (!$sphinx_connection = sphinx_search_connect()) return false;

    if (!$sphinx_search_index = sphinx_search_index()) return false;
    ...
    if (!$result = db_query($sql, $db_sphinx_search_update_index)) return false;
...
function sphinx_search_delete_index($tid, $pid)
{
    if (forum_get_setting('sphinx_search_enabled', 'N')) return false;
