...
function session_update_user_time($uid)
{
    ...
    $sql.= "CAST('$current_datetime' AS DATETIME) AS USER_TIME_UPDATED FROM (SELECT USER_FORUM.UID, ";
    $sql.= "IF(COALESCE(USER_TRACK.USER_TIME_UPDATED, 0) > COALESCE(USER_FORUM.LAST_VISIT, 0), ";
    $sql.= "COALESCE(UNIX_TIMESTAMP(USER_TRACK.USER_TIME_UPDATED), 0), COALESCE(UNIX_TIMESTAMP(USER_FORUM.LAST_VISIT), 0)) AS TIME_START, ";
    $sql.= "IF(COALESCE(SESSIONS.TIME, 0) > COALESCE(USER_TRACK.USER_TIME_UPDATED, 0), ";
    $sql.= "COALESCE(UNIX_TIMESTAMP(SESSIONS.TIME), 0), COALESCE(UNIX_TIMESTAMP(USER_TRACK.USER_TIME_UPDATED), 0)) AS TIME_END, ";
    $sql.= "COALESCE(UNIX_TIMESTAMP(USER_TRACK.USER_TIME_TOTAL), 0) AS USER_TIME_TOTAL ";
    $sql.= "FROM SESSIONS INNER JOIN USER_FORUM ON (USER_FORUM.UID = SESSIONS.UID AND USER_FORUM.FID = SESSIONS.FID) ";
    $sql.= "LEFT JOIN `{$table_data['PREFIX']}USER_TRACK` USER_TRACK ON (USER_TRACK.UID = USER_FORUM.UID) ";
    $sql.= "WHERE SESSIONS.UID = '$uid' AND SESSIONS.FID = '$forum_fid') AS USER_TIME_CALC ";
