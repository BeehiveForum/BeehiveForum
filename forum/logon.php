...
/* $Id: logon.php,v 1.110 2004-03-10 18:43:17 decoyduck Exp $ */
...
if (isset($HTTP_GET_VARS['deletecookie']) && $HTTP_GET_VARS['deletecookie'] == 'yes') {
    ...
    if (isset($HTTP_SERVER_VARS['SERVER_SOFTWARE']) && !strstr($HTTP_SERVER_VARS['SERVER_SOFTWARE'], 'Microsoft-IIS')) {
        ...
        header_redirect("./index.php?webtag=$webtag". (isset($final_uri) ? '?final_uri='. urlencode($final_uri) : ''));
        ...
        echo "<!--\n";
        echo "document.location.href = './index.php?webtag=$webtag", (isset($final_uri) ? '?final_uri='. urlencode($final_uri) : ''), "';\n";
        echo "//-->\n";
        echo "</script>";
...
if (isset($HTTP_POST_VARS['submit'])) {
    ...
    if (isset($HTTP_POST_VARS['logon']) && isset($HTTP_POST_VARS['password'])) {
        ...
        if (isset($luid) && $luid > -1) {
            ...
            echo "<p class=\"smalltext\"><a href=\"logon.php?webtag=$webtag&deletecookie=yes", (isset($final_uri) ? '&final_uri='. urlencode($final_uri) : ''), "\" target=\"_top\">{$lang['deletecookies']}</a></p>\n";
	    echo "<p class=\"smalltext\"><a href=\"forgot_pw.php?webtag=$webtag", (isset($final_uri) ? '?final_uri='. urlencode($final_uri) : ''), "\" target=\"_self\">{$lang['forgottenpasswd']}</a></p>\n";
...
echo "  <p class=\"smalltext\">{$lang['donthaveanaccount']} <a href=\"register.php?webtag=$webtag", (isset($final_uri) ? '?final_uri='. urlencode($final_uri) : ''), "\" target=\"_self\">Register now.</a></p>\n";
echo "  <hr width=\"350\" />\n";
echo "  <h2>{$lang['problemsloggingon']}</h2>\n";
echo "  <p class=\"smalltext\"><a href=\"logon.php?webtag=$webtag&deletecookie=yes", (isset($final_uri) ? '&final_uri='. urlencode($final_uri) : ''), "\" target=\"_top\">{$lang['deletecookies']}</a></p>\n";
echo "  <p class=\"smalltext\"><a href=\"forgot_pw.php?webtag=$webtag", (isset($final_uri) ? '?final_uri='. urlencode($final_uri) : ''), "\" target=\"_self\">{$lang['forgottenpasswd']}</a></p>\n";
